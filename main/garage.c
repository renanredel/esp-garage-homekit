...
void current_door_state_update_from_sensor(){
	...
	// FAST VERSION TO CHECK THE CURRENT STATE OF THE DOOR
void current_door_state_update_light(){
	adc1_config_width(ADC_WIDTH_12Bit);	
	int open = 0;
	int close = 0;
	for (int i = 0; i<5; i++){
...
homekit_value_t gdo_current_state_get() {
	...
	printf("Update from sensor");
    }else{
		current_door_state_update_light();
	}
...
void gdo_target_state_set(homekit_value_t new_value) {
	...
	led_write(true);
	printf("ATIVANDO CONTROLE");
	vTaskDelay(400 / portTICK_PERIOD_MS);
	led_write(false);
	//	TURN ON THE CONTROLLER #TODO
